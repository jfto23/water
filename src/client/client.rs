...
use crate::{
    camera::PlayerMarker,
    character::{build_player_ent, movement_2, Health, NetworkScenario},
    client::reconciliation::reconcile,
    consts::{CLIENT_SNAPSHOT_HISTORY_LENGTH, ROCKET_SPEED},
    input::Action,
...
use super::{
    reconciliation::{
        debug_reconciliation_event, save_snapshot, ReconciliationEvent, Snapshot, SnapshotHistory,
    },
    sync::{
        check_handshake_progress, finalize_sync, ping_server, LastServerTick, RoundTripTimes,
        SyncState, TickOffset,
    },
};
    ...
    fn build(&self, mut app: &mut App) {
        ...
        app.add_systems(
            ...
            (
                save_snapshot,
                update_tick,
                // todo disable inputs when pressing escape
        ...
        app.add_systems(
            FixedUpdate,
            (ping_server, check_handshake_progress)
                .in_set(Connected)
                .run_if(in_state(SyncState::Sync)),
        );

        app.add_systems(OnEnter(SyncState::Done), (finalize_sync));
        app.init_resource::<RoundTripTimes>();
        app.init_resource::<TickOffset>();
        app.init_resource::<LastServerTick>();
        app.init_state::<SyncState>();
        ...
        app.add_event::<ReconciliationEvent>();
        app.add_systems(Update, debug_reconciliation_event);
    }
}
...
fn receive_message_system(
    ...
    mut server_tick: ResMut<LastServerTick>,
    tick_offset: Res<TickOffset>,
    mut reconciliation_event: EventWriter<ReconciliationEvent>,
    mut round_trips: ResMut<RoundTripTimes>,
) {
    ...
    while let Some(message) = client.receive_message(ServerChannel::ServerMessages) {
        ...
        match server_message {
            ...
            ServerMessages::Pong {
                ...
                let rtt = since_the_epoch.as_millis() - initial_timestamp;
                round_trips.history.push(rtt)
            }
        }
    }
    ...
    while let Some(message) = client.receive_message(ServerChannel::NetworkedEntities) {
        ...
        client_tick.current_tick = networked_entities.server_tick + tick_offset.offset as u64;
        for i in 0..networked_entities.entities.len() {
            if let Some(entity) = network_mapping.0.get(&networked_entities.entities[i]) {
                ...
                if lobby
                    ...
                    && snapshot_history.history.len() > 0
                {
                    let (rec, snapshot) = reconcile(
                        &translation,
                        &snapshot_history,
                        networked_entities.server_tick,
                    );
                    if rec {
                        reconciliation_event.send(ReconciliationEvent {
                            old: snapshot,
                            new: translation,
                        });
                    ...
                    // for other players, we just forcefully update their positions without reconciliation
                    player_tf.translation = translation;
                    *player_velocity = velocity;
                }
        ...
        if let Some(owner) = action_diff_event.owner {
            ...
            action_diff_event.action_diffs.iter().for_each(|diff| {
                ...
                let input_message = bincode::serialize(&ClientAction {
                    ...
                    client_tick: client_tick.current_tick,
                })
                .unwrap();
